...
public class EventoUniversitario implements Serializable {
    ...
    public void crearActividad(int id, String titulo, int cupo, int nivel) {
        Curso nuevoCurso = new Curso(id, titulo, cupo, nivel);
    ...
    public <T extends Actividad> List<T> filtrarActividadesPorTipo(Class<T> tipo){
        System.out.println("Las actividades del tipo "+tipo.getSimpleName() +" son: ");
        List<T> actividadFiltrada = new ArrayList<>();
        for(Actividad actividad:this.getActividades()){
            if(tipo.isInstance(actividad)){
                actividadFiltrada.add((T)actividad);
    ...
    public double calcularCostoMateriales(List<? extends Actividad> actividades){
        if (actividades == null || actividades.isEmpty()) {
            System.out.println("No hay actividades en la lista proporcionada. Costo de materiales: 0.0");
            return 0.0;
        }
        double costoActividad = 0;
        for(Actividad actividadLista : actividades){
            costoActividad += actividadLista.calcularCostoMateriales();
        }
        System.out.println("La cantidad de actividades del tipo " + actividades.get(0).getClass().getSimpleName()
                + " es de " + actividades.size() + " y el costo de materiales es de " + costoActividad);
