...
public class App {
    public static void main(String[] args) {
        try {
            ...
            eventosCreados.add(evento1);
            eventosCreados.add(evento2);
            eventosCreados.add(evento3);
            ...
            // d. Se confirman ALGUNAS de las inscripciones
            evento1.getActividades().get(0).getInscripcionesActividad().get(0).setEstado("Confirmada");
            evento1.getActividades().get(1).getInscripcionesActividad().get(0).setEstado("Confirmada");
            evento2.getActividades().get(0).getInscripcionesActividad().get(0).setEstado("Confirmada");
            evento3.getActividades().get(0).getInscripcionesActividad().get(0).setEstado("Confirmada");
            ...
            for (EventoUniversitario evento : eventosCreados) {
                for (Actividad actividad : evento.getActividades()) {
                    for (Inscripcion inscripcion : actividad.getInscripcionesActividad()) {
                        if ("Confirmada".equalsIgnoreCase(inscripcion.getEstado())) {
                            String idTicket = "TCK-" + evento.getId() + "-" + inscripcion.getEstudiante().getLegajo();
                            Inscripcion.TicketDeAcceso nuevoTicket = inscripcion.new TicketDeAcceso(idTicket);
                            inscripcion.setTicket(nuevoTicket);
                            System.out.println("Ticket " + idTicket + " generado para " + inscripcion.getEstudiante().getNombre());
                        }
                    }
                }
            }
            ...
            System.out.println("----------------------------------------------------");
            System.out.println("ENVIANDO TICKETS (HILO SECUNDARIO) Y MOSTRANDO DATOS (HILO PRINCIPAL)");
            for(EventoUniversitario evento : eventosCreados){
                EnvioTicketsThread envioTickets = new EnvioTicketsThread(evento);
                envioTickets.start();

                System.out.println("[" + Thread.currentThread().getName() + "] Mostrando datos del evento " + evento.getTitulo());
                evento.mostrarDatos();
                for(Actividad actividad : evento.getActividades()){
                    for(Inscripcion inscripcion : actividad.getInscripcionesActividad()){
                        System.out.println("[" + Thread.currentThread().getName() + "] " + actividad.getTitulo() + " - inscripto: " + inscripcion.getEstudiante().getNombre() + " (" + inscripcion.getEstado() + ")");
                    }
                }
            }
            ...
            evento1.getActividades().get(0).getInscripcionesActividad().get(0).setEstado("Confirmada");
            evento1.getActividades().get(0).getInscripcionesActividad().get(1).setEstado("Confirmada");
